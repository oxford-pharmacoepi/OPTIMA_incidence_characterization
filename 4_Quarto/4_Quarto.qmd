...
char_4 <- comorb_characteristics %>%
  filter(group_level == "Lung Cancer All",
         ...
         estimate_name %in% c("count", "percentage"),
         strata_name == "overall",
         variable_level %in% c("Venous thromboembolism", "Asthma", "Heart failure", "Copd",
...
char_5 <- med_characteristics %>%
  filter(group_level == "Lung Cancer All",
         ...
         estimate_name %in% c("count", "percentage"),
         strata_name == "overall",
         variable_level %in% c("Drugs acid related disorder", "Lipid modifying agents", "Drugs used in diabetes", "Immunosupressants",
...
lsc_cats <- subset(lsc_cats, !is.na(`Lung Cancer All Sampled`) & !is.na(`Lung Cancer All Matched`)) %>%
  filter(`Standardised mean difference` >= 0.15 | `Standardised mean difference` <= -0.15) %>%
  na.omit()

ggplot(lsc_cats, aes(x = `Lung Cancer All Sampled`, y = `Lung Cancer All Matched`, color = Database)) +
  geom_point(aes(shape = `Signs and Symptoms`)) +
  scale_shape_manual(values = 0:25) +
  geom_label(data = subset(lsc_cats, `Standardised mean difference` >= 0.4 | `Standardised mean difference` <= -0.4 | `Lung Cancer All Sampled` >= 50 | `Lung Cancer All Matched` >= 50),
            aes(label = `Concept name (concept ID)`),
            vjust = -0.5, size = 3) +
  labs(
    title = "Prevelance of signs and symptoms in Lung Cancer Cohort and Matched Cohort (-365 to -31)",
    y = "Matched",
    x = "Sampled"
  ) +
...
ggplot(lsc_risks, aes(x = `Lung Cancer All Sampled`, y = `Lung Cancer All Matched`, color = Database)) +
  geom_point(aes(shape = `Risk Factors`)) +
  scale_shape_manual(values = 0:25) +
  geom_label(data = subset(lsc_risks, `Standardised mean difference` >= 0.4 | `Standardised mean difference` <= -0.4 | `Lung Cancer All Sampled` >= 50 | `Lung Cancer All Matched` >= 50),
            aes(label = `Concept name (concept ID)`),
            vjust = -0.5, size = 3) +
  labs(
    title = "Prevelance of risk factors in Lung Cancer Cohort and Matched Cohort (-365 to -31)",
    y = "Matched",
    x = "Sampled"
  ) +
  geom_abline(intercept = 0, slope = 1, linetype = "dotted", color = "black") +
...
ggplot(lsc_met, aes(x = `Lung Cancer All Sampled`, y = `Lung Cancer All Matched`, color = Database)) +
  geom_point(aes(shape = `Metabolic Syndrome`)) +
  scale_shape_manual(values = 0:25) +
  geom_label(data = subset(lsc_met, `Standardised mean difference` >= 0.4 | `Standardised mean difference` <= -0.4 | `Lung Cancer All Sampled` >= 25 | `Lung Cancer All Matched` >= 25),
            aes(label = `Concept name (concept ID)`),
            vjust = -0.5, size = 3) +
  labs(
    title = "Prevelance of metabolic syndromes in Lung Cancer Cohort and Matched Cohort (-365 to -31)",
    y = "Matched",
    x = "Sampled"
  ) +
  geom_abline(intercept = 0, slope = 1, linetype = "dotted", color = "black") +
