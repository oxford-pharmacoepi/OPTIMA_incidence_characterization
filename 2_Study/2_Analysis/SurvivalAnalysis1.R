...
if(cdm$death %>% head(5) %>% count() %>% pull("n") > 0){
  ...
  surv <- estimateSingleEventSurvival(cdm = cdm,
                                      followUpDays = 3653,
                                      timeGap = c(365, 1096, 1826, 3653) ,
                                      targetCohortTable = "mm_cohort",
                                      targetCohortId = mm_surv_cohort_id,
                                      outcomeCohortTable = "mm_death",
                                      strata = list(c("sex"),
                                                    c("age_group")),
                                      minCellCount = 5)
  
  surv_study_p <- estimateSingleEventSurvival(cdm = cdm,
                                              followUpDays = 1096,
                                              timeGap = c(365, 1096) ,
                                              targetCohortTable = "mm_cohort",
                                              targetCohortId = mm_surv_cohort_id,
                                              outcomeCohortTable = "mm_death",
                                              strata = list(c("study_period")),
                                              minCellCount = 5)
  surv_study_p<- surv_study_p %>%
    filter(strata_level != "Overall")
  # export survival ----
  write_csv(bind_rows(surv,surv_study_p),
            here("Results", paste0(
              "survival_", cdmName(cdm), ".csv"
            )))
}
